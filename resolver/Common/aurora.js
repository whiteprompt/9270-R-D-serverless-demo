...
exports.getUser = async (client, uuid) => {
    let user = {};
    let userFromDb = await client.query(`select id, uuid, name from users where uuid = ?`, [uuid]);

    if (userFromDb.length === 0) {
        return null;
    }

    let postsFromDb = await client.query(`select uuid, text from posts where user_id = ?`, [userFromDb[0].id]);
